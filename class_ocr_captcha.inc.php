...
//    function ocr_captcha($long=4,$lx=120,$ly=30,$nb_noise=25) {
  function ocr_captcha($long=4,$lx=120,$ly=50,$nb_noise=5) {
    ...
    function get_filename($public="") { 
      ...
      if (!is_dir("../Suporte/tmp")) // test if rep exist
        mkdir("../Suporte/tmp"); 
      if (strpos($_SERVER['SystemRoot'], ":\\")===false) // so linux system
        $rad="../Suporte/tmp/"; // Document_root works nicely here  
      else // windows system 
        $rad="../Suporte\tmp\\"; 
...
//      $size=intval(rand(12,17)); 
        $size=intval(rand(18,22));
        $angle=intval(rand(-30,30));
        $text=strtoupper(substr($private_key,$i,1));
        imagettftext ($image,$size,$angle,$x+2,34,$shadow,$this->font_file,$text);
        imagettftext ($image,$size,$angle,$x,32,$color,$this->font_file,$text);
        $x+=$size+2;
      } 
    ...
    function display_captcha($noise=true) {
      ...
      $alt=($this->lang=="fr")?("Vous devez recopier dans le champ ci-dessous les ".$this->long." caractères parmis 0 à 9 et A à F"):("Vous must read and type the ".$this->long." chars within 0..9 and A..F");
