...
$sql = "SELECT DRD, MAX(Path + 0) AS maxpath, MAX(Row + 0) AS maxrow, MIN(Path + 0) AS minpath, MIN(Row + 0) AS minrow FROM Scene
        INNER JOIN $tabprefix" . "Scene ON Scene.SceneId=" . $tabprefix . "Scene.SceneId WHERE Satellite='$satellite' AND Sensor='$sensor' AND Date='$date' and Deleted <> 99  GROUP BY DRD";
...
$sql = "SELECT * FROM Scene INNER JOIN $tabprefix" . "Scene ON Scene.SceneId=" . "$tabprefix" . "Scene.SceneId
        WHERE Satellite='$satellite' and Sensor='$sensor' and Date='$date' and Deleted <> 99  ORDER BY DRD ASC";
